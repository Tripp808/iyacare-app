...
async function addRealtimePatient() {
  try {
    ...
    const realtimePatient = {
      ...
      notes: "🔴 REAL-TIME IoT MONITORING PATIENT - Connected to live ESP32 sensor devices",
      ...
      iotDeviceId: "ESP32-001-IMM",
      realtimeDatabasePath: "/readings",
    ...
    console.log('👩 Name: Immaculée Munezero (Rwandan)');
    console.log('🔗 IoT Device ID: ESP32-001-IMM');
    console.log('📡 Real-time Database Path: /readings');
    console.log('🏥 This patient will receive live IoT data from your ESP32 device');
...
addRealtimePatient()
  .then(() => {
    ...
    console.log('🔴 Immaculée Munezero is now connected to live IoT readings');
    process.exit(0);
  })
  .catch((error) => {
    console.error('\n💥 Script failed:', error);
